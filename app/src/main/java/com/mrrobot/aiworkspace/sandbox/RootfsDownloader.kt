...
/**
 * Downloads and extracts the Alpine Linux mini-rootfs.
 * Uses HttpURLConnection (no Ktor dependency). Ported from Kai (Apache-2.0).
 */
object RootfsDownloader {
    ...
    private fun extractTar(inputStream: java.io.InputStream, targetDir: File) {
        ...
        while (true) {
            ...
            val name = readTarString(headerBuffer, 0, 100)
            if (name.isEmpty()) break

            val prefix = readTarString(headerBuffer, 345, 155)
            val fullName = if (prefix.isNotEmpty()) "$prefix/$name" else name

            val sizeStr = readTarString(headerBuffer, 124, 12)
            val size = if (sizeStr.isNotEmpty()) sizeStr.toLong(8) else 0L

            val modeStr = readTarString(headerBuffer, 100, 8)
            val mode = if (modeStr.isNotEmpty()) modeStr.toInt(8) else 0
            val typeFlag = headerBuffer[156]
            val linkName = readTarString(headerBuffer, 157, 100)
