...
// emitted to stderr so user redirects of stdout don't swallow it. We use
// Kotlin Unicode escapes here so the source stays readable; on the bash
// side we emit the same bytes via octal `\036` / `\037` for portability
// across bash and busybox printf.
private const val RS = "\u001e"
private const val US = "\u001f"

// Marker emitted once at shell startup so we know bash's pid before any
// user command has finished. Without this, cancel on the first command
// had nothing to signal (bashPid was null, set only from the sentinel
// of a completed run).
private const val PID_PROBE_PREFIX = "${RS}MRBASHPID$US"

/**
 * One long-lived `bash --noprofile --norc` per logical session. State
 * (cwd, env vars, sourced scripts, aliases, shell variables) survives
 * between commands — so the AI can `cd` once, run a sequence of
 * commands, and have them all execute in the same directory.
 *
 * Ported from Kai's `PersistentSandboxShell` (Apache-2.0). The wire
 * protocol on stderr is byte-compatible with Kai's so any future
 * hardening upstream there can be folded back here unchanged.
 *
 * Concurrency: [run] is mutex-serialised. The class is safe to share
 * across coroutines as long as callers respect that contract.
 *
 * Failure modes:
 *  - bash dies → in-flight command completes with `shellDied = true`,
 *    `handle` is cleared, next [run] lazily restarts.
 *  - command times out → graduated SIGINT/SIGTERM/SIGKILL to bash's
 *    children via a sibling proot. If escalation doesn't free us, the
 *    whole shell is reset.
 *  - sentinel never arrives → withTimeoutOrNull catches the wait and
 *    triggers the same recovery path.
 */
class PersistentSandboxShell(
     ...
     * sentinel is observed, the per-command timeout fires, or the
     * shell dies. Concurrent calls are serialised by an internal mutex.
     *
     * Returns a Map with keys: `success` (Boolean), `stdout` (String),
     * `stderr` (String), `exit_code` (Int), `timed_out` (Boolean),
     * `cwd` (String), `shell_died` (Boolean). Shape matches
     * [ProotExecutor.execute] so callers can switch back and forth.
     */
    suspend fun run(
        command: String,
        timeoutSeconds: Long,
        ...
        if (result == null) {
            // Hung command. Try a graduated cancel; if that doesn't shake
            // it loose within a short grace, reset the shell.
            cancelForeground()
    ...
    /**
     * Forward a stdin line to the running command. The shell's stdin
     * is also the foreground command's stdin (no redirection), so this
     * delivers interactive input (e.g. ssh password prompts) to the
     * running process.
     */
    ...
    /**
     * Best-effort interrupt of the foreground command without killing
     * the shell itself. Without a PTY we can't deliver SIGINT through
     * line discipline, so we send signals to bash's children from a
     * sibling proot. Falls back to a full shell reset if the pid isn't
     * known yet (probe race) or if even SIGKILL doesn't free the
     * foreground.
     */
    fun cancelForeground() {
        val pid = bashPid
        if (pid == null) {
            // No pid captured yet — the user expects cancel to actually
            // do something, so nuke the shell. The next run lazily
            // restarts it.
            reset()
            return
        }
        scope.launch {
            for (signal in listOf("INT", "TERM", "KILL")) {
                ...
                // Stop escalating as soon as the in-flight command
                // finishes (sentinel arrived) or there's no in-flight
                // command anymore.
                val done = currentSink.get()?.done?.isCompleted
                if (done == null || done == true) return@launch
            }
            // Even SIGKILL didn't free us — the shell itself must be
            // wedged. Last resort.
            reset()
        }
    }
    ...
    private fun ensureShell() {
        ...
        // Non-interactive bash. We have no tty, so -i would only emit
        // prompts to stderr that we'd have to filter. --noprofile
        // /--norc keep the env clean; bash still reads commands from
        // stdin line by line, executes them in-process (so cd/export/.
        // preserve state), and inherits its stdin to any foreground
        // child (so ssh can read passwords typed via writeInput).
        ...
        handle = h
        // Capture bash's pid before any user command runs. The
        // dispatcher recognises this marker on stderr and sets bashPid,
        // so cancel on the very first command has something to signal.
        // Leading \n matches the sentinel pattern below — flushes any
        // partial line first.
        h.writeInput("printf '\\n\\036MRBASHPID\\037%d\\036\\n' \"\$\$\" >&2")
        watchdog = scope.launch {
            h.awaitExit()
            // Shell died. Wake up any in-flight command with a
            // shellDied result so callers don't sit on a sentinel that
            // will never come.
            currentSink.getAndSet(null)?.done?.complete(
    ...
    private fun dispatchStderr(line: String) {
        ...
        // to flush any partial line ahead of it, which produces a
        // stray empty line when there's nothing to flush. Legitimate
        // blank stderr is rare; dropping it is a worthwhile tradeoff
        // for clean output.
        if (line.isEmpty()) return
        // Startup pid probe — handled regardless of whether a sink is
        // active.
    ...
    private fun sendSignalToChildren(parentPid: Int, signal: String) {
        // pgrep/pkill come from busybox and are present in the base
        // rootfs. We use pgrep + xargs kill because some busybox builds
        // don't have pkill -P. Failure is swallowed; this is best-
        // effort.
        runCatching {
            executor.execute(
    ...
    private fun buildResult(
        ...
        return mapOf(
            ...
            "stdout" to smartTruncate(sink.stdoutBuf.toString()),
            "stderr" to smartTruncate(stderr),
    ...
    private fun timeoutMap(sink: CommandSink, stderr: String): Map<String, Any> = mapOf(
        "success" to false,
        "stdout" to smartTruncate(sink.stdoutBuf.toString()),
        "stderr" to smartTruncate(sink.stderrBuf.toString() + "\n" + stderr),
        "exit_code" to -1,
        "timed_out" to true,
        "cwd" to "/root",
        "shell_died" to true
    ...
    private fun smartTruncate(text: String): String {
        ...
        return text.take(MAX_OUTPUT_LENGTH / 2) +
            "\n...[truncated]...\n" +
            text.takeLast(MAX_OUTPUT_LENGTH / 2)
    }
}
